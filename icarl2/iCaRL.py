...
class iCaRL:
    ...
    def __SKLClassifier__(self, data, exemplars, net, n_classes, classifier):
        ...
        loader = DataLoader(
            data, batch_size=512, shuffle=False, num_workers=4, drop_last=False
        )
    ...
    def run(self, train_batches, test_batches, net, herding=True, classifier="NME"):
        ...
        for idx, batch in enumerate(train_batches):
            ...
            # Classification
            if classifier == "NME":
                accuracy, predictions, labels = utils.NMEClassifier(
                    test_batches[idx], batch, exemplars, net, n_classes, self.device
                )
            elif classifier == "FC":
                accuracy, predictions, labels = utils.FCClassifier(
                    test_batches[idx], net, n_classes, self.device
                )
            else:
                accuracy, predictions, labels = self.__SKLClassifier__(
                    test_batches[idx], exemplars, net, n_classes, classifier
                )
